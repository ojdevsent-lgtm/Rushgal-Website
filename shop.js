...
function updateCheckoutSummary(){
  const sub=subtotal();const delivery=state.fulfilment==='delivery'?state.deliveryFee:0;
  $('#checkoutSubtotal').textContent=money(sub);$('#deliveryFee').textContent=delivery?money(delivery):'FREE';$('#checkoutTotal').textContent=money(sub+delivery);
  $('#deliveryFields').classList.toggle('is-hidden',state.fulfilment!=='delivery');
}
...
function requestLocation(){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{ $('#latitude').value=pos.coords.latitude.toFixed(6);$('#longitude').value=pos.coords.longitude.toFixed(6);status.textContent='✓ Location captured. Rushgal can use it to confirm the delivery area.'; },()=>{status.textContent='We could not access your location. You can enter the address manually.'},{enableHighAccuracy:true,timeout:10000});
}

async function placeOrder(event){
  event.preventDefault();
  const form=event.currentTarget;const data=new FormData(form);const submit=form.querySelector('.place');const status=$('#checkoutStatus');
